...
export function createShop({ economy, config, extraEffects, stock }) {
  ...
  // CREATIVE MODE, as one number and one flag. The flag is read in exactly one
  // place in this module -- priceOf() -- and every other path (canBuy, buy,
  // refund, refundQuote, listTab) already goes through it, so making everything
  // free is a change to what a price IS rather than a bypass of the money check.
  // buy() still calls economy.spend(price); it just spends 0.
  const creativeCfg = (config && config.creative) || {};
  const creativePrice = typeof creativeCfg.price === 'number' ? creativeCfg.price : 0;
  let creative = !!creativeCfg.enabled;
  ...
  function priceOf(id, ownedLevel) {
    const row = byId(id);
    if (!row) return null;
    // The single point of decision for creative mode's "everything is free".
    // It is deliberately ABOVE the level and curve maths: a free upgrade does
    // not climb a price ladder, and a refund quote off a free purchase is 0,
    // which is what stops demolish from printing money.
    if (creative) return creativePrice;
  ...
  function rerollStock() {
    ...
    // The shelf's own price, quoted through the same creative rule as every
    // other price in this module. A reroll costing money in a session where
    // nothing else does would be the one line in the mode that surprises you.
    const price = creative ? creativePrice : shelf.rerollCost;
    if (!economy.canAfford(price)) return refuse('insufficient', price);
  ...
  // --- what a row is actually worth, per minute --------------------------------
  //
  // Every producer row describes itself in WORDS ("stamps out a duck every few
  // seconds"), which is unreadable as a comparison: two machines cannot be
  // ranked without buying both. That is how five rows ended up strictly
  // dominated by cheaper ones without anybody noticing.
  //
  // Nothing new is invented here. The numbers are the row's own produce block
  // folded with the live stat table -- the same two things the running game
  // uses to decide what the machine does -- so a column can never disagree with
  // the machine standing on the plate.
  //
  // A row with no `produce` (a belt, a fan, a collector, an upgrade, a wall)
  // gets null, and null means "this does not make ducks", which is a different
  // statement from 0 and must stay distinguishable to whatever draws it.

  // The AVERAGE duck value for a weight set, which is what turns ducks a minute
  // into money a minute. rarityLuckMul scales every tier above the first exactly
  // as src/sim/ducks.js rolls it.
  function meanRarityMultiplier(setName, luckMul) {
    const r = (config && config.rarity) || null;
    if (!r || !Array.isArray(r.multipliers)) return 1;
    const name = creative ? ((config.creative && config.creative.rarityWeights) || setName) : setName;
    const weights = (r.sets && r.sets[name]) || r.weights;
    if (!Array.isArray(weights)) return 1;
    let wsum = 0;
    let vsum = 0;
    for (let i = 0; i < r.multipliers.length; i++) {
      const w = (weights[i] || 0) * (i > 0 ? luckMul : 1);
      wsum += w;
      vsum += w * r.multipliers[i];
    }
    return wsum > 0 ? vsum / wsum : 1;
  }

  // produce.count is 1, a number, or a [min, max] range the machine rolls per
  // cycle. A range averages, because that is what "per minute" means.
  function ducksPerCycle(p) {
    if (!p || p.count === undefined || p.count === null) return 1;
    if (Array.isArray(p.count)) {
      const a = Number(p.count[0]) || 0;
      const b = Number(p.count[1]) || 0;
      return (a + b) / 2;
    }
    return Number(p.count) || 1;
  }

  function ratesOf(row, stats) {
    const p = row && row.produce;
    // Only a machine that runs ON ITS OWN has a rate. The workbench and the
    // Auto-Cranker are driven by hands on a wheel -- one player's, several
    // players', a bot's -- so their output is a property of who is standing
    // there, and quoting a number would be inventing one.
    if (!p || !p.secondsPerDuck || row.kind !== 'producer_auto') {
      // `note` is why there is no number, as a machine name rather than a
      // sentence: 'hands' means somebody has to work it, null means the row
      // does not make ducks at all. The words are the UI's to choose.
      const hands = row && (row.kind === 'producer_manual' || row.kind === 'crank_bot');
      return { ducksPerMinute: null, moneyPerMinute: null, note: hands ? 'hands' : null };
    }
    const rateMul = (stats && stats.machineRateMul) || 1;
    const seconds = p.secondsPerDuck / (rateMul > 0 ? rateMul : 1);
    if (!(seconds > 0)) return { ducksPerMinute: null, moneyPerMinute: null, note: null };
    const dpm = (60 / seconds) * ducksPerCycle(p);
    const econ = (config && config.economy) || {};
    const base = typeof econ.duckBaseValue === 'number' ? econ.duckBaseValue : 1;
    const valueMul = (stats && stats.duckValueMul) || 1;
    const mean = meanRarityMultiplier(p.rarityWeights, (stats && stats.rarityLuckMul) || 1);
    return { ducksPerMinute: dpm, moneyPerMinute: dpm * base * valueMul * mean, note: null };
  }
  ...
  function listTab(tab) {
    ...
    // One stat read for the whole tab: it is the same table for every row and
    // computeStats() walks every owned level each time it is called.
    const stats = computeStats(ownedLevelTable(), extra());
    const out = rows.map((row) => {
      const c = canBuy(row.id);
      const m = maxLevelOf(row);
      const info = shelf ? shelf.info(row.id) : null;
      const rates = ratesOf(row, stats);
      return {
        // Derived from the row's own produce block and the live stat table.
        // null on anything that does not make ducks by itself.
        ducksPerMinute: rates.ducksPerMinute,
        moneyPerMinute: rates.moneyPerMinute,
        rateNote: rates.note,
  ...
  return {
    ...
    stock: shelf,
    priceOf,
    // The session's mode, set once by whoever owns the session (the host, from
    // the lobby) before play starts. Not a per-player toggle and not something
    // gameplay code calls: it is here so the flag has ONE owner per module.
