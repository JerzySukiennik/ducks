...
export function createStock({ rows, config, random }) {
  ...
  // CREATIVE MODE. The shelf keeps rolling exactly as it always did -- the
  // rarity model, the period and the countdown are untouched -- and the ONE
  // thing that changes is what unitsOf() answers. Every other function in this
  // file already asks unitsOf(), so "nothing is ever unavailable" is a single
  // point of decision rather than a flag threaded through take/give/info.
  const creativeUnits = Math.max(1, Math.round(num((config && config.creative || {}).stockUnits, 999)));
  let creative = !!((config && config.creative || {}).enabled);
  ...
  function unitsOf(id) {
    if (creative) {
      const m = meta.get(id);
      // A row with no price is not sold through the shop at all, so it is not
      // "out of stock" in creative either -- it was never on the shelf. Giving
      // it units would invent a state nothing reads, exactly as rollOne() says.
      if (!m) return 0;
      return m.cost > 0 ? creativeUnits : 0;
    }
  ...
  function take(id, n) {
    // In creative the shelf is bottomless: the units are not decremented, so a
    // row still reads its full count after the purchase and the next player
    // through the door sees the same shelf.
    if (creative) return unitsOf(id) > 0;
  ...
  function give(id, n) {
    ...
    // Nothing was taken, so there is nothing to put back. Reported as accepted
    // because the caller's invariant ("the unit is back on the shelf") holds.
  ...
  return {
    ...
    info(id) {
      ...
      return {
        ...
        units: unitsOf(id),
        // `rolled` is "what this period started with", and in creative that is
        // the same bottomless number -- a panel drawing "3 / 14 left" must not
        // show a full bar against a stale cap.
        rolled: creative ? unitsOf(id) : (rolled.has(id) ? rolled.get(id) : 0),
      };
    },
    // The session's mode. Set once by the session's owner before play starts;
    // nothing in the shelf's own logic ever calls it.
    setCreative(on) { creative = !!on; notify('creative'); return creative; },
    isCreative: () => creative,
    creativeUnits,
