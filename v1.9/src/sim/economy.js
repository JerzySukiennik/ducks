...
export function createEconomy({ start = 0, duckBaseValue = 1, duckValueMul = 1 } = {}) {
  ...
  function spend(n, reason) {
    ...
    // Spending NOTHING succeeds and moves nothing. This is what lets a free
    // purchase go down the same path as a paid one: the shop still calls
    // spend() with the price it quoted, the price happens to be 0, the money
    // does not move and no listener is told about a change that did not happen.
    // The alternative -- letting the shop skip the call when the price is 0 --
    // is exactly the "money check skipped somewhere" that makes a summary
    // screen and a wallet disagree.
    if (v === 0) return true;
    if (v < 0) return false;
