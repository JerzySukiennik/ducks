...
export function createBlowers({ config, ducks, applyImpulse, list, byId }) {
  ...
  function describe(rec, row) {
    ...
    // +sin, not -sin. The MESH is rotated by build.js yawQuaternion(), which is
    // three's rotation about +Y, so a model's local +Z lands on
    // (sin yaw, 0, cos yaw). This used to be -sin, which is a rotation by MINUS
    // yaw -- the same at 0 and 180 degrees and MIRRORED IN X at every other
    // angle. A fan turned 90 degrees blew out of its own back, and because the
    // airstream was invisible nobody could see it: measured, a duck on the side
    // the fan visibly faced travelled 0.000 m. The renderer was right and the
    // physics was wrong, so the physics moved.
