...
// The fix is columns, not decoration. There are now four, in the order the
...
#shop-cols, .shop-row { display: grid;
  grid-template-columns: 96px 1fr 64px 108px; gap: 14px; align-items: center;
  padding: 0 18px; }
...
export function createShopUI({
  ...
  function renderPrestige() {
    ...
    presBtn.disabled = !armed && !confirming;
    presBtn.title = armed ? '' : (allow.reason || q.reason || '');
    ...
    doIt.disabled = !armed;
    doIt.title = armed ? '' : (allow.reason || q.reason || '');
  }
  ...
  function render() {
    ...
    for (let i = 0; i < rows.length; i++) {
      ...
      // 3. STOCK. `null` means the shop was built without a shelf at all, which
      ...
      // 4. ONE button, one state. When it is off it says WHY, because a dead
  ...
  return {
    ...
    shelfState: () => (shelf ? {
      ...
      rerollEnabled: !rerollBtn.disabled,
