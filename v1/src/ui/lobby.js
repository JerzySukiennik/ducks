...
#lobby-players { display: flex; flex-direction: column; gap: 4px; }
.lobby-player { display: flex; gap: 8px; align-items: baseline; }
.lobby-player .sl { color: ${CRT.dim}; font-size: ${CRT.type.label.size}; min-width: 52px; }
.lobby-player .nk { color: ${CRT.bright}; }
...
export function createLobbyUI(opts) {
  ...
  async function host() {
    ...
    session = res.session;
    players = [{ slot: 0, nick: session.nick, self: true }];
  ...
  async function join(roomId) {
    ...
    session = res.session;
    players = [
      { slot: 0, nick: session.hostNick || 'host' },
      { slot: session.slot, nick: session.nick, self: true },
    ];
    say('Joined.');
    renderSession();
    syncButtons();
  ...
  return {
    ...
    open: doOpen,
    close: doClose,
    toggle(options) { return open ? (doClose(), Promise.resolve(false)) : doOpen(options); },
    isOpen: () => open,
    refresh,
    host,
    join,
