...
// Splits a baked geometry in two. The predicate is asked about whole connected
// PARTS, not single triangles: the crank's spokes are long bars that straddle
// any plane you would cut on, so a per-triangle test would leave half a spoke
// welded to the cabinet while the rest turned.
function partition(geometry, predicate) {
  ...
  // Weld by position, then join the three corners of every triangle.
  const seen = new Map();
  ...
  acc.forEach((s, r) => verdict.set(r, !!predicate(s[0] / s[3], s[1] / s[3], s[2] / s[3])));

  const inside = [];
  const outside = [];
  for (let t = 0; t < tris; t++) {
    (verdict.get(find(t * 3)) ? inside : outside).push(t);
  }
  function build(list) {
  ...
  return [build(inside), build(outside)];
}
