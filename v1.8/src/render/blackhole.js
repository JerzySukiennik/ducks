...
// starfield.
//
// PORTED from ../Gargantua/src/scene/blackhole.js. Every line of the shader
// below -- hash/vnoise/fbm, the disk, the halo, the photon ring, the star
// layers, the lensing itself -- is that file's, unchanged. It is a verified
// artefact and none of the physics is ours to improve.
//
// FOUR of its constants are not Gargantua's any more: RS, DISK_OUT and BEAM
// were retuned to make this hole bigger and heavier than Gargantua's, and the
// reasoning for each is written at the constant block itself. The rule followed
// there is the one that keeps the picture coherent: RS is the only statement of
// scale, everything else is a RATIO to it, so the ratios that must not change
// (the photon ring, the halo band, the disk's inner edge) were left alone.
//
// Only the JS around it differs, and only in three ways, all forced by this
...
// Gargantua's scale. Every radius below except FLAT is expressed in units of
// RS -- the shader works in rN = b / RS -- so RS is the ONE constant that says
// how big the hole is and the rest say what the hole looks like. Making it
// bigger therefore means raising RS and leaving the ratios alone, which is the
// only way the picture stays a coherent black hole rather than a shadow with
// somebody else's ring around it.
//
//   RS       0.115 -> 0.190. The shadow disc, in units of the quad's half
//            width. 1.65x wider on the quad; with the quad itself enlarged
//            (config.sky.holeSize/holeDistance) that is 5.3 -> 12.8 degrees of
//            apparent diameter. This is the mass knob: everything else follows.
//   HALO_IN  1.02, HALO_OUT 1.8, and the photon ring's own 1.06 in main() are
//            UNCHANGED, because they are ratios to the shadow. They therefore
//            grow with RS automatically and the photon ring stays welded to the
//            shadow edge exactly where it was.
//   DISK_IN  1.8, unchanged for the same reason: the disk's inner edge stays at
//            the same physical radius above the horizon.
//   DISK_OUT 7.2 -> 4.6. This one HAD to move. The disk's outer edge sits at
//            DISK_OUT * RS of the half-quad and the quad's own edgeFade begins
//            at 0.88, so at the old ratio the enlarged disk (7.2 * 0.190 =
//            1.37) would have been cut off by the edge of its own quad. 4.6
//            puts it at 0.874, which is where 7.2 * 0.115 = 0.828 used to sit:
//            the disk keeps its framing and is now a more compact, brighter
//            torus wrapped tight around a much bigger shadow, which is what a
//            heavier hole looks like.
//   BEAM     0.42 -> 0.52. Relativistic beaming scales with the orbital speed
//            of the gas that is emitting, and truncating the disk at 4.6 RS
//            instead of 7.2 means the light now comes from deeper in the well,
//            where the gas is faster. A harder bright/dark asymmetry across the
//            disk is the visible consequence, and it is what makes the thing
//            read as heavy instead of merely large.
const float RS = 0.190;
const float FLAT = 0.20;
const float DISK_IN = 1.8;
const float DISK_OUT = 4.6;
const float HALO_IN = 1.02;
const float HALO_OUT = 1.8;
const float BEAM = 0.52;
