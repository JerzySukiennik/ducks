...
// CREATIVE MODE's weight set, resolved by NAME out of config exactly the way
// src/sim/producers.js resolves a producer row's set. An unknown name is fatal
// rather than a silent fallback, for the same reason it is there: a mode that
// quietly did nothing would be indistinguishable from a mode that was off.
export function creativeWeightSet(config) {
  const name = config && config.creative && config.creative.rarityWeights;
  const sets = config && config.rarity && config.rarity.sets;
  const w = sets && sets[name];
  if (!Array.isArray(w) || !w.length) {
    throw new Error(
      `[ducks] creative rarity weight set '${name}' is not in config.rarity.sets. ` +
      `Known sets: ${sets ? Object.keys(sets).join(', ') : '(none)'}`
    );
  }
  return w;
}
...
export function createDucks({ RAPIER, world, cfg, groups, counters, rng }) {
  ...
  // CREATIVE MODE, as data: a rarity weight set, or null for the normal game.
  // When one is set every tier this pool assigns is rolled from it -- including
  // a tier handed in by a caller. That last part is deliberate and it is the
  // whole reason this lives HERE rather than in each duck source: producers,
  // containers and the snapshot restore all pass a tier they rolled or
  // remembered elsewhere, and a creative session in which the presses still
  // made common ducks would be a mode that half worked. Re-rolling is
  // idempotent inside a creative session, because every tier that reaches this
  // function was itself rolled from this same set.
  //
  // The one case it is not idempotent: restoring a save made in a NORMAL
  // session into a creative one promotes the restored ducks. That is the mode
  // doing what it says rather than a bug, and it is recorded here so nobody
  // has to rediscover it from a value readout.
  let creativeWeights = null;

  function setCreativeWeights(w) {
    if (!w) { creativeWeights = null; return null; }
    if (!Array.isArray(w) || w.length !== cfg.multipliers.length) {
      throw new Error(
        `[ducks] a creative weight set needs exactly ${cfg.multipliers.length} weights, one per tier`
      );
    }
    creativeWeights = w.slice();
    return creativeWeights;
  }

  // The ONLY place a tier becomes real. Both spawn paths go through it.
  function tierFor(tierOrNull) {
    if (creativeWeights) return pickTier(creativeWeights, random);
    if (typeof tierOrNull !== 'number' || !isFinite(tierOrNull)) return pickTier(cfg.weights, random);
    return tierOrNull;
  }
  ...
  return {
    ...
    pose,
    forEach,
    // The session's mode. `config` is the whole config object, so the pool
    // resolves the set by name itself and no caller has to know which set
    // creative uses. Set once, before play starts, by whoever owns the session.
    setCreative(config, on) {
      return setCreativeWeights(on ? creativeWeightSet(config) : null);
    },
    setCreativeWeights,
    isCreative: () => !!creativeWeights,
    creativeWeights: () => (creativeWeights ? creativeWeights.slice() : null),
