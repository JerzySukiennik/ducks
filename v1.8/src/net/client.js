...
export function createClient({ session, game, onEvent }) {
  ...
  const counters = {
    ...
    inputRecordsSent: 0,
    staleBodies: 0,
    // Convergence. `ghostsCollected` is the count of ducks this tab was drawing
    // that the host does not have -- every one of them was untouchable, and a
    // number that keeps climbing during play means something on this side is
    // still creating bodies the host never hears about.
    livenessIn: 0,
    livenessBad: 0,
    livenessSuspended: 0,
    ghostsCollected: 0,
  ...
  function onControl(msg) {
    ...
    switch (msg.t) {
      case MSG.WELCOME:
        joined = true;
        // Which side of the door we arrived on. 'lobby' -> wait with everybody
        // else; 'playing' -> the session is already running and this tab goes
        // straight into the world with no intro. The host decides this, never
        // the client, which is why it arrives in WELCOME rather than being
      ...
      case MSG.LIVENESS: applyLiveness(msg); return;
  ...
  // The host's periodic statement of the WHOLE duck set, against which this
  // tab's own pool is compared. It exists because every other message about a
  // duck is said once: "it spawned", "it is gone". Both of those ride the
  // reliable channel and neither is lost in transit -- but this client also
  // runs its own copy of the simulation, and that copy can create a duck the
  // host never had (a producer on a placed machine ticks here too) or destroy
  // one the host still has (a container absorbing it). Neither shows up as a
  // lost message anywhere, and after it happens nothing in the diff protocol
  // will ever mention that slot again: the host believes it has already told
  // this client the truth, and it did.
  //
  // Extra ducks are collected here, because a duck the host does not have is a
  // duck nobody can ever push, score or pick up -- it is scenery pretending to
  // be a body. Missing ducks cannot be repaired here at all: this tab has no
  // tier and no pose for them, so it asks, and the host re-announces them
  // through the same path a spawn takes.
  let lastResyncAt = -1e9;
  // When the intro started on this tab, by this tab's clock. See
  // net.livenessCutsceneGraceMs: the cutscene is the ONE thing that is supposed
  // to put ducks in this pool that the host never sent, and striking them
  // mid-shot would be a worse bug than the one this whole mechanism fixes.
  let cutsceneAt = -1e9;

  function livenessSuspended() {
    const phase = typeof game.phase === 'function' ? game.phase() : 'playing';
    if (phase !== 'playing') return 'lobby';
    if (now() - cutsceneAt < Math.max(0, Math.round(N.livenessCutsceneGraceMs))) return 'cutscene';
    return null;
  }

  function applyLiveness(msg) {
    if (!ready) return null;
    const why = livenessSuspended();
    if (why) { counters.livenessSuspended++; return { suspended: why }; }
    const pool = game.ducks;
    if (!pool || typeof pool.forEach !== 'function') return null;
    const n = Math.max(0, msg.n | 0) || pool.max || 0;
    const live = decodeLiveBitmap(msg.bits, n);
    if (!live) { counters.livenessBad++; return null; }
    counters.livenessIn++;
    // Collected first, applied second: forEach walks the pool and removeBody
    // mutates it.
    const ghosts = [];
    const mine = new Uint8Array(n);
    pool.forEach((id) => {
      if (id >= 0 && id < n) mine[id] = 1;
      if (id >= 0 && id < n && !live[id]) ghosts.push(id);
    });
    for (let i = 0; i < ghosts.length; i++) {
      const id = ghosts[i];
      if (game.removeBody(WIRE_KIND.DUCK, id)) counters.ghostsCollected++;
      track.delete(wire.duck(id));
    }
    const missing = [];
    const max = Math.max(1, Math.round(N.livenessMaxResyncIds));
    for (let i = 0; i < n && missing.length < max; i++) {
      if (live[i] && !mine[i]) missing.push(i);
    }
    if (!missing.length) return { ghosts: ghosts.length, missing: 0 };
    const t = now();
    // Rate limited on this end as well as on the host's: one unanswered ask per
    // interval, not one per bitmap.
    if (t - lastResyncAt < Math.max(0, Math.round(N.livenessResyncCooldownMs))) {
      return { ghosts: ghosts.length, missing: missing.length, asked: false };
    }
    lastResyncAt = t;
    if (peer.sendControl({ t: MSG.RESYNC, ids: missing })) counters.resyncSent++;
    counters.missingSeen += missing.length;
    return { ghosts: ghosts.length, missing: missing.length, asked: true };
  }
  ...
  function applyEvent(msg) {
    ...
    switch (msg.e) {
      ...
      case EV.GAMBLE: game.applyGamble(msg); return;
      ...
      // The host pressed Start. `msg.at` is the host's session clock at that
      // instant; `hostClock` is the newest host clock this tab has seen on the
      // state stream. The difference is how far into the timeline this tab
      // already is, which is what makes every tab run ONE cutscene instead of
      // each free-running its own.
      // The intro is about to stage a set in this tab's own duck pool, so the
      // liveness sweep stands down for the length of it. See livenessSuspended.
      case EV.CUTSCENE: cutsceneAt = now(); game.applyCutscene(msg, hostClock); return;
      default: return;
    }
  }
  ...
  const off = session.on((ev) => {
    ...
    if (ev.type === 'open') {
      // The colour this player picked rides on HELLO beside the nick. The host
      // folds it into the roster, which is the one place every tab already reads
      // a player's facts from, so the waiting room draws everybody in their own
      // colour rather than in the colour of their slot.
      peer.sendControl({
        t: MSG.HELLO, v: PROTOCOL.version, nick: session.nick, slot: mySlot,
        color: (typeof game.localColor === 'function' ? game.localColor() : null),
      });
