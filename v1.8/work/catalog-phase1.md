...
Spisany PRZED implementacją, bo dwa buildery pracują równolegle: jeden pisze kod
i walidator, drugi wiersze danych. Bez jednego kształtu wiersze nie przeszłyby
walidacji, a rozjazd wyszedłby dopiero przy starcie.

Każde pole jest **opcjonalne**. Wiersz bez niego zachowuje się dokładnie tak jak
dziś — to jest warunek, żeby 31 istniejących wierszy nie wymagało ani jednej
zmiany.

### 1. `produce.count` — ile kaczek na emisję

```js
produce: { secondsPerDuck: 30, rarityWeights: 'w_basic', count: 3 }
produce: { secondsPerDuck: 30, rarityWeights: 'w_basic', count: [1, 10] }
```

Liczba całkowita ≥ 1, albo para `[min, max]` losowana przy każdej emisji.
Brak pola = 1. Emisja przy pełnej puli **wypuszcza tyle, ile się zmieści, resztę
porzuca** i podnosi flagę zacięcia — nigdy nie kolejkuje długu, bo zaległość
wypuszczona później zalałaby plac w jednej klatce.

### 2. `collider.restitution` i `collider.friction`

```js
collider: { shape: 'cuboid', half: [...], blockDucks: true, restitution: 0.85 }
```

Obie w zakresie 0–2. Brak = wartości domyślne świata (`friction` 0.9,
`restitution` domyślna Rapiera).

**Poprawka do tego kontraktu, wyszła przy implementacji:** domyślna restytucja
Rapiera to **0**, więc `restitution` może iść tylko W GÓRĘ. Nie da się nią
stłumić odbicia — wartość 0.02 jest nieodróżnialna od braku pola. Ściana
tłumiąca musi to robić **wysokim `friction`**, nie niską restytucją. Kto
wystroi tu 0.02 licząc na miękkość, dostanie dokładnie zero efektu i nie
zobaczy błędu. `world.addStaticBox` **już przyjmuje
`box.friction`** — brakuje wyłącznie przekazania ich z wiersza przez
`render/placed.js`.

### 3. `blow.pitchDegrees` — nadmuch pod kątem

```js
blow: { force: 26, range: 6, cone: 35, pitchDegrees: 90 }
```

Zakres −90…+90, dodatnie = w górę. Brak = 0, czyli poziomo, jak dziś. Liczony
względem **postawionej pozy**, tak samo jak `collider.surface.pitchDegrees`
przy rampach — nie w przestrzeni świata.

### 4. `storage.leakPerSecond` — pojemnik, który gubi

```js
storage: { capacity: 40, tipToEmpty: true, leakPerSecond: 0.2 }
```

Kaczek na sekundę wypadających z **niesionego albo pchanego** pojemnika.
Brak = 0. Wypadają realnie (wirtualna zamienia się w ciało), więc gracz je widzi
i może podnieść — inaczej byłby to niewidzialny wyciek pieniędzy, czyli dokładnie
ta klasa błędu, którą wycięliśmy z eviction w G1.

### 5. `tool.pull` i nowy tryb

```js
tool: { mode: 'sweep', reach: 1.8, arc: 110, force: 6, pull: true }
tool: { mode: 'scoop', reach: 1.2, arc: 90, force: 0, capacity: 5 }
```

`pull: true` odwraca znak siły — narzędzie przyciąga zamiast odpychać. Brak =
`false`. Nowy tryb `scoop` dochodzi do zamkniętej listy `KINDS.tool.modes`
(`['sweep','beam','scoop']`): podnosi do `capacity` kaczek jednym ruchem, bez
pojemnika. Nieznany tryb pozostaje błędem startu.

## Czego pilnować przy wierszach

- **Ceny i krzywe muszą przejść przez `tools/economy-sim.py`.** 49 nowych pozycji
  zmienia katalog, a faza E dobierała progi tak, żeby wyczerpał się dokładnie na
  koniec sesji. Wpisanie 49 cen „na oko" psuje tę krzywą po cichu.
- **Nie ruszać 31 istniejących wierszy.** Ich liczby są zmierzone i zamrożone.
- **Wiatrak: `force` musi bić próg tarcia** μ·g = 0,6 × 22 = **13,2**. Wartość
  poniżej jest dokładnie zerem, a wygląda jak strojenie. Dotyczy też każdego
  narzędzia popychającego.
