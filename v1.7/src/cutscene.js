...
export function createCutscene(opts) {
  ...
  function pumpSettle(maxSteps) {
    ...
    if (settle.settled || settle.steps >= cap) settle.done = true;
    return settle;
  }
  ...
  function finishSettle() {
    while (settle && !settle.done) pumpSettle(Math.round(C.settleStepsPerFrame) * 8);
    return settle;
  }
  ...
  function startMusic(offsetSeconds) {
    ...
    const nodes = bus.synthNodes ? bus.synthNodes() : null;
