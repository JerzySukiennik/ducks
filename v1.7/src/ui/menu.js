...
export function createMenuUI(opts) {
  ...
  el('div', null, brand, 'terminal zakładowy').id = 'menu-eyebrow';
  const title = el('h1', null, brand, 'Ducks');
  title.id = 'menu-title';
  el('span', 'crt-cursor', title);
  el('div', null, brand).id = 'menu-rule';
  const tag = el('div', null, brand, 'System gotowy. Wybierz tryb pracy.');
  tag.id = 'menu-tag';
  ...
  function pane(titleText) {
    ...
    const back = el('button', 'menu-back', head, 'Wróć');
    back.type = 'button';
  ...
  const startWorld = kv(startPane.body, 'Świat');
  const startDucks = kv(startPane.body, 'Kaczki');
  const startPlayer = kv(startPane.body, 'Gracz');
  const startSave = kv(startPane.body, 'Zapis');
  const startNote = el('div', 'menu-note', startPane.body, '');
  const startBtn = act(acts(startPane.body), 'go', 'Graj', () => doClose('play'));

  // --- 02 Wspólnie ----------------------------------------------------------
  const roomPane = pane('Pokój');
  const roomStatus = kv(roomPane.body, 'Status');
  const roomId = kv(roomPane.body, 'Pokój');
  const roomVis = kv(roomPane.body, 'Widoczność');
  el('div', 'menu-note', roomPane.body,
    'Sieć nigdy nie stoi między tobą a graniem: jeśli serwer pokoi nie odpowiada, '
    + 'lobby powie to jednym zdaniem, a gra dla jednego gracza działa dalej.');
  act(acts(roomPane.body), null, 'Otwórz lobby', () => {
    if (o.onMultiplayer) o.onMultiplayer();
  });

  // --- 03 Sterowanie --------------------------------------------------------
  const controlsPane = pane('Klawisze');
  const controls = createControlsPanel({ mount: controlsPane.body });

  // --- 04 Ustawienia --------------------------------------------------------
  const setPane = pane('Ustawienia');
  const setVol = kv(setPane.body, 'Głośność');
  const setSens = kv(setPane.body, 'Czułość myszy');
  const setFov = kv(setPane.body, 'Pole widzenia');
  const setBuf = kv(setPane.body, 'Pikseloza');
  const setNick = kv(setPane.body, 'Nick');
  const setColor = kv(setPane.body, 'Kolor');
  act(acts(setPane.body), null, 'Zmień ustawienia', () => {
    if (o.onSettings) o.onSettings();
  });

  // --- 05 Zakończ sesję -----------------------------------------------------
  const endPane = pane('Koniec');
  const endThrown = kv(endPane.body, 'Wrzucone');
  const endEarned = kv(endPane.body, 'Zarobek');
  const endBuilt = kv(endPane.body, 'Zbudowane');
  const endWho = kv(endPane.body, 'Sesja');
  el('div', 'menu-note', endPane.body,
    'Nic nie jest zapisywane. Zakończenie pokazuje podsumowanie i zamyka pokój.');
  const endBtn = act(acts(endPane.body), 'stop', 'Zakończ sesję', () => {
    if (o.onEndSession) o.onEndSession();
  });

  navItem('01', 'Graj sam', 'jeden gracz, bez sieci', startPane.node);
  navItem('02', 'Wspólnie', 'załóż pokój albo dołącz', roomPane.node);
  navItem('03', 'Sterowanie', 'pełna lista klawiszy', controlsPane.node);
  navItem('04', 'Ustawienia', 'dźwięk, mysz, obraz', setPane.node);
  navItem('05', 'Zakończ sesję', 'podsumowanie i wynik', endPane.node);
  ...
  // and it exists for one reason: the first action says Graj before the first
  // game and Wróć do gry afterwards, because "resume" when nothing has started
  ...
  function sync() {
    ...
    // 01 Start
    startWorld.textContent = 'plac ' + config.world.plateSize + ' m';
    startDucks.textContent = 'pula ' + config.ducks.max;
    startPlayer.textContent = nickname();
    startPlayer.appendChild(swatch());
    startSave.textContent = 'brak — sesja kończy się podsumowaniem';
    startNote.textContent = inRoom
      ? 'Jesteś w pokoju. Wyjście stąd wraca do gry i nie zamyka pokoju.'
      : started
        ? 'Świat cały czas chodzi za tym ekranem.'
        : 'Świat już chodzi za tym ekranem — to nie jest ładowanie.';
    startBtn.textContent = started || inRoom ? 'Wróć do gry' : 'Graj';

    // 02 Pokój
    roomStatus.textContent = inRoom
      ? (room.isHost ? 'prowadzisz pokój' : 'w cudzym pokoju')
      : 'jeden gracz';
    roomStatus.classList.toggle('ok', inRoom);
    roomId.textContent = inRoom ? String(room.roomId || '—') : '—';
    roomVis.textContent = inRoom
      ? (room.isPublic ? 'publiczny — na liście' : 'prywatny — tylko z linku')
      : '—';

    // 04 Ustawienia -- read live from the store, never a second copy.
    if (settings) {
      ...
      setBuf.textContent = 'bufor ' + Math.round(Number(settings.get('bufferWidth'))) + ' px';
      setNick.textContent = nickname();
      setColor.textContent = '';
      setColor.appendChild(swatch());
    }

    // 05 Koniec -- the same collector the summary screen reads, so the two
    ...
    endThrown.textContent = st ? num(st.ducksScored) + ' w dziurze' : '—';
    endEarned.textContent = st ? '$' + num(st.moneyEarned) : '—';
    endBuilt.textContent = st
      ? num((st.built || []).reduce((n, r) => n + r.count, 0)) + ' obiektów'
      : '—';
    endWho.textContent = inRoom
      ? (room.isHost ? 'kończysz ją wszystkim w pokoju' : 'wychodzisz z pokoju')
      : 'tylko ty';
    endBtn.textContent = inRoom && room.isHost ? 'Zakończ dla wszystkich' : 'Zakończ sesję';

    footVersion.textContent = 'rev ' + (o.version || config.version)
      + ' — ' + (o.degraded ? 'kinematyczny fallback fizyki' : 'fizyka gotowa');
    return true;
  }
