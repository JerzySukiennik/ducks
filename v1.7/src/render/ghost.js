...
export function createGhost({ scene, models }) {
  ...
  // hologram, because the fan's blow axis and the fan's mesh do not share a
  // frame: blowers.js drives the force off (-sin yaw, cos yaw) and the mesh is
  // yawed by three's own convention. Parenting the cone to the model would have
  // drawn it in the model's frame, i.e. it would have agreed with the picture
  // and disagreed with the physics -- the one failure this hint exists to
  // prevent. See streamCheck() in placed.js for the same test on a placed fan.
