...
export function createAvatarsView(opts) {
  ...
  mesh.count = 0;
  // At most three other players, so the shadow pass gains one instanced draw.
  // Receiving is on here (unlike the ducks): a person-sized body is big enough
  // for a wall's shadow falling across it to be visible.
  mesh.castShadow = !!config.world.shadowsEnabled;
  mesh.receiveShadow = !!config.world.shadowsEnabled;
