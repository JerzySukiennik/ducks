...
export const config = {
  ...
  render: {
    ...
    clearColor: 0x0a0f1e,
    fogColor: 0x0a0f1e,
    // Fog is what the 180 m plate hands the horizon instead of a hard concrete
    // edge against the stars. fogNear stays where gameplay is -- nothing inside
    // 55 m is touched, and the whole 35 m walk from the bench to the pit is
    // fog-free -- while fogFar is pulled in from 190 to 150 so the far plate
    // edge (90 m dead ahead, 127 m at the corners) is 27% and 76% faded into
    // the sky colour. At the old 190 the new edge sat at 23% and read as a line.
    fogNear: 55,
    fogFar: 150,
  ...
  world: {
    ...
    sunColor: 0xfff2dd,
    // Shadows are on, so these two are now a RATIO as well as two brightnesses:
    // the sun is what a shadow removes and the hemisphere is what is left
    // standing in it. The plate's normal is straight up and sunDir normalises to
    // y = 0.878, so a lit patch of concrete gets 1.75 * 0.878 = 1.54 of sun on
    // top of 0.44 of sky, and the same patch in shadow gets the 0.44 alone --
    // about 22% of the lit value. Dark enough that a duck's shadow is a shape
    // rather than a smudge, bright enough that a shadow is never a black hole in
    // the floor. Moving one of these without the other changes the contrast of
    // every shadow in the game.
    sunIntensity: 1.75,
    hemiIntensity: 0.44,
    ...
    shadowsEnabled: 1,
    // 1024 over a 2 * 34 = 68 m box is 6.6 cm per texel: a duck (20 cm) is three
    // texels across, a wall is thirty. 2048 would be prettier and costs four
    // times the fill; measured at 300 ducks it was not worth it.
    shadowMapSize: 1024,
    // Half-width of the fitted box, in metres. 34 covers everything the player
    // can build (build.maxDistance is 14) plus the walk in front of them.
    // Anything beyond it simply stops casting -- at a 480 px backbuffer a shadow
    // 34 m away is under two pixels wide.
    shadowRadius: 34,
    // How far up the sunDir the shadow camera sits. It only has to clear the
    // tallest caster (the chute mouth at 7 m); 90 leaves the ortho depth range
    // comfortable without wasting precision.
    shadowDistance: 90,
    // Acne vs peter-panning. normalBias does the work here because every surface
    // in this game is flat-shaded with hard normals; the constant bias is small
    // and only cleans up the grazing case across the plate.
    shadowBias: -0.0006,
