...
export const MSG = {
  ...
  // The whole authoritative duck set, re-stated on a slow timer. Every other
  // message about a duck is a one-shot announcement -- "it exists", "it is
  // gone" -- and a one-shot announcement that fails to take effect on a client
  // is never repeated, because the host's digest has already moved on. This is
  // the statement that cannot go stale, and it is what makes a divergence
  // temporary no matter what caused it.
  LIVENESS: 'duckLive',           // host -> everyone, bitmap of live pool slots
  // A client answering that bitmap: "you say these slots are alive and I do not
  // have them". It carries no pose and no tier -- it cannot, that is the whole
  // reason it has to ask -- so it is a question, exactly like every REQ.
  RESYNC: 'duckResync',           // client -> host, slots it is missing
...
export const EV = {
  ...
  HOTBAR: 'hotbar',
  // The host pressed Start in the lobby. It carries ONE number: the host's own
  // session clock at the instant it started. Every client subtracts that from
  // the host clock it is currently seeing and enters the timeline at the same
  // point, so four tabs run one cutscene rather than four. Nothing here carries
  // a duration or a shot list -- both ends read the same beat file.
  CUTSCENE: 'cutscene',
  // A gambling box started, opened or finished a roll.
  GAMBLE: 'gamble',
};
...
export const REQ = {
  ...
  // nobody else can see.
  REROLL: 'reroll',
  // The workbench wheel, HELD rather than clicked: `{down:true, o, d}` on the
  // press and `{down:false}` on the release, exactly the shape REQ.TOOL uses,
  // and for the same reason -- a held button is two edges and the host owns
  // everything between them. It carries NO workbench id: the aim travels as
  // evidence like GRAB's does, the host casts the wheel test itself from its own
  // belief about that player's eye, and the reach check is re-run every tick
  // against the wheel it decided on. A client that could name a bench could name
  ...
  POUR: 'pour',
  // Pay to roll the gambling box the asker is looking at. The prize is the
  // HOST's dice, like the vendor's shelf: a client that rolled its own would be
  // looking at a reward nobody else can see.
  GAMBLE: 'gamble',
  // --- the waiting room -------------------------------------------------------
  // A room now opens into a LOBBY, not into the world, and these are the two
  // verbs that live there.
  //
  // READY is a client saying "I am set" -- it carries nothing but a boolean and
  // the colour that player picked, and the host folds both into the roster every
  // other tab already reads. It is a request rather than a broadcast for the
  // usual reason: the host owns the roster, so the host is what says a player is
  // ready, and a client that could assert its own ready flag could assert
  // anybody's.
  READY: 'ready',
  // START is the host's alone -- it is what begins the session for the whole
  // room -- and it is refused for any other slot with that reason, exactly as
  // PRESTIGE is. Everyone can see the lobby; one person opens the door.
  START: 'start',
};
...
// --- the liveness bitmap ------------------------------------------------------
// One bit per duck pool slot, LSB first, base64'd so it survives the JSON
// control channel. 300 slots is 38 bytes of bitmap and 52 characters of text --
// the entire authoritative duck set, cheaper than three duck poses.
//
// Both ends share these two functions for the same reason encodeInput and
// decodeInput sit next to each other: when the writer and the reader of a
// packed format live in two files, a change to one of them reads as data rather
// than as an error.
...
function toBase64(bytes) {
  ...
  // No btoa (a bare test runner). Same output, written out by hand.
  let out = '';
...
// `bits` is anything indexable that answers truthy for a live slot.
export function encodeLiveBitmap(bits, count) {
...
// Returns a Uint8Array of 0/1 per slot, or null if the text is not readable.
// Never throws: a malformed bitmap must not be able to stop a client's frame.
export function decodeLiveBitmap(text, count) {
