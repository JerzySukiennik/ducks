...
export function createProducers({ ducks, applyImpulse, list, byId, config, statsOf, rng }) {
  ...
  function update(dt) {
    ...
    for (const u of units.values()) {
      ...
      while (u.timer >= interval && budget > 0) {
        if (ducks.atCap()) {
          // STOP, do not delete and do not bank the debt: hold the timer one
          // interval short so the first duck after the jam clears is immediate
          // but the ones missed while jammed never arrive as a burst.
          u.timer = interval;
          refusedAtCap++;
          jam = true;
          break;
        }
        if (emit(u) === null) { u.timer = interval; refusedAtCap++; jam = true; break; }
        u.timer -= interval;
        made++;
        budget--;
      }
  ...
  return {
    ...
    expectedPerMinute() {
      sync();
      let total = 0;
      for (const u of units.values()) total += 60 / intervalOf(u);
      return total;
    },
