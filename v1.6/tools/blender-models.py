...
def finish(objs, name, bevel=0.0, merge=0.0):
...
def b_crank():
    P = []
    P.append(box("body", (1.00, 0.80, 1.15), (0, 0, 0.575), "teal"))
    P.append(box("skirt", (1.08, 0.88, 0.14), (0, 0, 0.07), "dark"))
    P.append(box("hood", (1.02, 0.84, 0.26), (0, 0, 1.28), "teal_lt"))
    ...
    P.append(cyl("wheel", 0.34, 0.07, (0.58, 0, 0.78), "hazard", verts=14, rot=(0, math.radians(90), 0)))
    P.append(cyl("hub", 0.10, 0.13, (0.60, 0, 0.78), "dark", verts=10, rot=(0, math.radians(90), 0)))
    for i in range(4):
        P.append(box("spoke%d" % i, (0.06, 0.52, 0.05), (0.585, 0, 0.78), "steel",
                     rot=(math.radians(45+i*90), 0, math.radians(90))))
    P.append(cyl("knob", 0.06, 0.16, (0.66, 0.24, 1.02), "orange", verts=8, rot=(0, math.radians(90), 0)))
    for sx in (-1, 1):
        for sy in (-1, 1):
            P.append(box("foot", (0.10, 0.10, 0.09), (sx*0.42, sy*0.32, 0.045), "dark"))
...
def b_wall_high():
    ...
    for sx in (-1, 1):
        P.append(box("rib", (0.10, 0.20, 2.52), (sx*0.86, 0, 1.30), "steel_d"))
    P.append(box("band", (2.02, 0.17, 0.10), (0, 0, 1.45), "steel_d"))
...
def b_bucket():
    ...
    ring_xy("rim", R-0.03, N, 0.15, 0.045, 0.05, H-0.03, "steel", P)
...
def b_neon_ducks():
    """Neon DUCKS - swiecacy szyld na niskim murku. Dekoracja, nie przeszkoda."""
    P = []
    P.append(box("foot", (2.06, 0.22, 0.10), (0, 0, 0.05), "steel_d"))
    for sx in (-1, 1):                      # maszty
        P.append(box("mast", (0.09, 0.11, 1.30), (sx*0.90, 0, 0.68), "dark"))
    P.append(box("board", (1.86, 0.07, 0.62), (0, 0, 1.06), "dark"))
    P.append(box("bframe", (1.94, 0.05, 0.70), (0, 0.03, 1.06), "steel_d"))
    # piec liter jako slupki neonu - z 20 m czyta sie jako napis, nie jako litery
    for i in range(5):
        x = -0.72 + i*0.36
        P.append(box("gl%d" % i, (0.20, 0.04, 0.40), (x, -0.05, 1.08), "hazard"))
        P.append(box("gl%dt" % i, (0.24, 0.03, 0.06), (x, -0.06, 1.30), "white"))
    P.append(box("hood", (1.98, 0.20, 0.07), (0, -0.04, 1.42), "hazard"))
    for i in range(4):                      # rurki zasilania
        P.append(box("tube%d" % i, (0.03, 0.03, 0.30), (-0.54 + i*0.36, 0.06, 0.60), "steel_d"))
...
def b_lasso():
    """Lasso - zwiniety sznur na haku. Tryb beam, przyciaga pojedyncza kaczke."""
    P = []
    N = 14
    for j, (R, z) in enumerate(((0.21, 0.07), (0.19, 0.12), (0.17, 0.17))):
        for i in range(N):
            a = math.radians(i*360.0/N + j*9)
            P.append(box("c%d%d" % (j, i), (2*math.pi*R/N*1.5, 0.045, 0.045),
                         (math.sin(a)*R, math.cos(a)*R, z), "wood_lt", rot=(0, 0, -a)))
    P.append(cyl("knot", 0.07, 0.10, (0.20, 0.02, 0.20), "wood", verts=8))
    P.append(box("tail", (0.05, 0.26, 0.05), (0.20, -0.14, 0.20), "wood_lt", rot=(math.radians(24), 0, 0)))
    P.append(cyl("hook", 0.035, 0.18, (0.20, -0.24, 0.10), "steel", verts=7, rot=(math.radians(70), 0, 0)))
    P.append(box("grip", (0.07, 0.16, 0.07), (-0.16, 0.10, 0.24), "rust"))
