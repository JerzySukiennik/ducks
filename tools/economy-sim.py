...
# A rung is only available once its item is owned. The sim always uses the best
# rung it owns, which is what a player does after the first time they try it.
...
CARRY_RUNGS = [
    ...
    ('bucket',   8,   1.0),
    ('box',     16,   2.0),
    ('cart',    24,   4.0),   # rolls, but you push it at walking pace
    ('box_big', 30,   6.0),   # heavy: the trip itself costs more
]
...
def transport_seconds(rung, count, speed_mul=1.0):
    ...
    walk = (2 * MACHINE_Z) / (EFFECTIVE_SPEED * speed_mul) + HANDLING_S
    return trips * (walk + overhead)
...
def simulate(bal, minutes=SESSION_MINUTES, verbose=False):
    ...
    def best_rung():
        ...
        for r in CARRY_RUNGS:
            if r[0] == 'hands' or lvl(r[0]) > 0:
                scaled = (r[0], r[1] * cap_mul if r[0] != 'hands' else 1, r[2])
                if scaled[1] >= best[1]:
                    best = scaled
        return best

    def auto_production():
        """Ducks per second made without the player touching anything."""
        rate = 0.0
        rate += lvl('press') / bal.pressSeconds
        rate += lvl('machine') / bal.machineSeconds
        return rate
