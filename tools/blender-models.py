...
def _crate(w, d, h, t, post, name, nx=4, nz=3, frame="steel_d", slat="wood_lt", floor="wood"):
    ...
    P.append(box("bot", (w - 2*t, d - 2*t, t), (0, 0, t/2), floor))
    for sx in (-1, 1):
        X = sx*(w/2 - t/2)
        for j in range(nx + 1):
            yy = -d/2 + t + j*(d - 2*t)/nx
            P.append(box("vx", (t, barw, h), (X, yy, h/2), frame))
        for k in range(nz + 1):
            zz = t + k*(h - t)/nz
            P.append(box("hx", (th, d - 2*t + barw, barw), (X, 0, zz), slat))
    for sy in (-1, 1):
        Y = sy*(d/2 - t/2)
        for j in range(nx + 1):
            xx = -w/2 + t + j*(w - 2*t)/nx
            P.append(box("vy", (barw, t, h), (xx, Y, h/2), frame))
        for k in range(nz + 1):
            zz = t + k*(h - t)/nz
            P.append(box("hy", (w - 2*t + barw, th, barw), (0, Y, zz), slat))
    for sx in (-1, 1):
        for sy in (-1, 1):
